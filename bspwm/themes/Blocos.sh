...
# rofi ---------------------------------
change_rofi() {
	sed -i -e "s/DIR=.*/DIR=\"$1\"/g" "$rofi_path"/bin/launcher "$rofi_path"/bin/powermenu "$rofi_path"/bin/windows "$rofi_path"/bin/powermenu.sh "$rofi_path"/bin/themes
...
change_bar 'blocos' 'Iosevka Nerd Font:size=10;3' && "$polybar_path"/launch.sh		# STYLE | FONT

## Change colors in funct (ROFI)
change_rofi 'blocos' 'Iosevka 10' '0px' 'Papirus-Apps'								# STYLE/DIR | FONT | BORDER | ICON
