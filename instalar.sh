...
cp -r /pop_bspwm/fonts/* ~/.fonts

echo "########## Pasta polybar copiada ##########"
cp -r /pop_bspwm/polybar ~/.config && cd ~/.config/polybar && chmod 755 ~/.config/polybar/* -R

echo "########## Pasta rofi copiada ##########"
cp -r /pop_bspwm/rofi ~/.config && cd ~/.config/rofi && chmod 755 ~/.config/rofi/* -R

echo "########## Pasta sxhkd copiada ##########"
cp -r /pop_bspwm/sxhkd ~/.config && cd ~/.config/sxhkd && chmod 755 ~/.config/sxhkd/* -R

echo "########## Pasta bspwm copiada ##########"
cp -r /pop_bspwm/bspwm ~/.config && cd ~/.config/bspwm && chmod 755 ~/.config/bspwm/* -R

echo "########## Removendo botões de janela e aplicando tema PopOs ##########"
cp -rf /pop_bspwm/gtk-3.0/ ~/.config/ && chmod 755 ~/.config/gtk-3.0/* -R
